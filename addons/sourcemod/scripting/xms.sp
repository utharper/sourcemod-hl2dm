...
{	// I politely ask you not to remove this
	...
	PrintCenterText(client, "XMS (eXtended Match System) by harper - www.hl2dm.pro");
...
{
	if(GetClientCount(true) == 1) // everyone left (except sourcetv)
	{
		if(Gamestate != STATE_CHANGE)
		{
...
void LoadGamemode()
{
	...
	if(XMS_GetConfigString(servercmd, sizeof(servercmd), "ServerCommand", "GameModes", Gamemode)) ServerCommand(servercmd);

	XMS_GetConfigString(description, sizeof(description), "Name", "GameModes", Gamemode);
